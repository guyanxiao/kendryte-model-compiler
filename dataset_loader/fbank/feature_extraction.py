...
def wav2fbank(file_path, max_pad_len):
    ''' convert wav files to fbank'''
    data_input, _ = wav2feature([file_path], feature_type='logfbank', feature_dim=64,
                                energy=False, delta1=False, delta2=False)
    data_input = data_input[0, :max_pad_len, :].transpose()
    data_input = np.around(np.clip(data_input, -8, 8) * 16)
    pad_width = max_pad_len - data_input.shape[1]
    data_input = np.pad(data_input, pad_width=((0, 0), (0, pad_width)), mode='constant')
    return data_input
